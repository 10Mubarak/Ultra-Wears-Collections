...
const LookbookPage = () => {
  ...
  return (
    <div className="bg-[#0A0A0A] text-white min-h-screen">
      <div className="border-b border-white/10 pt-20 md:pt-32 pb-12 md:pb-16 px-4 md:px-8 lg:px-16">
        <span className="text-[8px] md:text-[10px] font-mono text-blue-500 tracking-[0.4em] md:tracking-[0.5em] uppercase block mb-4">Editorial_Archive // Vol.01</span>
        <h1 className="text-3xl sm:text-4xl md:text-7xl lg:text-9xl font-black italic uppercase tracking-tighter leading-[0.8]">THE <br /> UNSEEN.</h1>
      </div>
        ...
        {scenes.map((scene) => (
          <div key={scene.id} className={`max-w-7xl mx-auto px-8 lg:px-16 flex flex-col ${scene.pos === 'right' ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-center gap-16`}>
            ...
            <div className="w-full lg:w-1/3 space-y-8">
              <div>
                <span className="text-[8px] md:text-[10px] font-mono text-blue-500 tracking-[0.2em] md:tracking-[0.3em] uppercase block mb-2">{scene.subtitle}</span>
                <h3 className="text-2xl sm:text-3xl md:text-4xl lg:text-6xl font-black italic uppercase tracking-tighter leading-none mb-4">{scene.title}</h3>
                <p className="text-gray-400 text-xs md:text-sm leading-relaxed">Featured: <span className="text-white font-bold">{scene.product.name}</span></p>
              </div>
