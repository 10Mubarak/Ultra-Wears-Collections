...
const FootwearPage = () => {
  ...
  return (
    <div className="bg-[#0A0A0A] text-white min-h-screen selection:bg-blue-600">
      ...
      {/* 1. TECH-HEADER */}
      <div className="px-4 md:px-8 lg:px-16 pt-12 md:pt-20 pb-8 md:pb-12 border-b border-white/10 flex flex-col md:flex-row justify-between items-end gap-6">
        <div className="max-w-xl w-full">
          <span className="text-[8px] md:text-[10px] font-mono font-bold text-blue-500 tracking-[0.4em] md:tracking-[0.5em] uppercase block mb-4">
            Category_02 // Footwear
          </span>
          <h1 className="text-3xl sm:text-4xl md:text-6xl lg:text-8xl font-black italic uppercase tracking-tighter leading-none">
      ...
      <div className="py-8 md:py-12 border-b border-white/10 opacity-20 overflow-hidden">
        <p className="text-[6px] md:text-[8px] font-mono tracking-[0.5em] md:tracking-[1em] uppercase whitespace-normal md:whitespace-nowrap">
          Traction_Control / Impact_Response / Lateral_Stability / Energy_Return_System / Ultra_Wears_Studio
        </p>
      </div>

      {/* 4. THE ANATOMY SECTION */}
      <section className="py-12 md:py-24 bg-[#0A0A0A] border-t border-white/10 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 md:px-8 lg:px-16">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-20 items-center">
            <div className="order-2 lg:order-1">
              <span className="text-[8px] md:text-[10px] font-mono text-blue-500 tracking-[0.3em] md:tracking-[0.4em] uppercase">Detailed_Anatomy // 001</span>
              <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-7xl font-black italic uppercase tracking-tighter mt-4 mb-10">
      ...
      <section className="bg-black py-12 md:py-24 px-4 md:px-8 lg:px-16 border-t border-white/10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-16 items-start">
          <div className="lg:col-span-3 order-2 lg:order-1 space-y-8 md:space-y-12">
            <div className="space-y-4 md:space-y-6">
              <h3 className="font-black text-sm md:text-lg lg:text-xl uppercase italic border-l-[6px] border-blue-600 pl-4 tracking-tight">Specs_Sheet</h3>
              <div className="space-y-3 md:space-y-5">
                {['DYNEEMA® REINFORCED', 'V-FRAME STABILIZER', 'MEGA-GRIP VIBRAM', 'RAPID-LOCK SYSTEM'].map((spec, i) => (
                  <div key={i} className="border-b border-white/5 pb-2 md:pb-3 font-black text-xs md:text-sm uppercase tracking-wide">
                    {spec}
                  </div>
                ))}
              </div>
          ...
          <div className="lg:col-span-3 order-3 space-y-10">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {['40', '41', '42', '43', '44', '45'].map((size) => (
                <button key={size} className="py-3 md:py-4 text-[9px] md:text-[11px] font-black border border-white/10 hover:border-blue-600 hover:text-blue-500 transition-all uppercase">
                  {size}
                </button>
              ))}
            </div>
            <div className="space-y-2 md:space-y-3">
              <button
                ...
                className="w-full bg-white text-black py-5 md:py-7 font-black uppercase text-base md:text-xl tracking-tighter hover:bg-blue-600 hover:text-white transition-all flex justify-between px-6 md:px-10 items-center group"
              >
              ...
              <button
                ...
                className="w-full py-3 md:py-4 border border-white/10 text-[8px] md:text-[9px] font-mono text-gray-500 uppercase tracking-[0.15em] md:tracking-[0.2em] hover:text-blue-500 hover:border-blue-500/50 transition-all"
              >
                [ + Save_To_Data_Bank ]
              </button>
            </div>
...
const ShoeCard = ({ shoe, onAdd, onArchive }) => (
  <div className="group relative border-r border-b border-white/10 overflow-hidden cursor-crosshair bg-black">
    <div className="absolute top-4 md:top-8 left-4 md:left-8 z-10">
      <span className="text-2xl md:text-4xl lg:text-[40px] font-black italic text-white/5 group-hover:text-blue-500/10 transition-colors">
        {shoe.id.split('-')[1]}
      </span>
    </div>

    <div className="aspect-square p-6 sm:p-12 md:p-20 flex items-center justify-center">
      <img
        src={shoe.img}
        alt={shoe.name}
    ...
    <div className="p-4 md:p-8 border-t border-white/10 bg-black/50 backdrop-blur-sm relative z-20">
      <div className="flex justify-between items-start mb-4 md:mb-6 gap-4">
        <div className="min-w-0 flex-1">
          <h3 className="text-sm md:text-lg lg:text-xl font-black italic uppercase tracking-tight leading-tight">{shoe.name}</h3>
          <p className="text-[7px] md:text-[9px] font-mono text-blue-400 uppercase tracking-widest mt-1 truncate">
            {shoe.tech}
          </p>
        </div>
        <span className="text-sm md:text-base lg:text-lg font-bold font-mono whitespace-nowrap">{shoe.price}</span>
      </div>

      <div className="flex gap-2">
        <button
          onClick={() => onAdd(shoe)}
          className="flex-[2] py-3 md:py-4 border border-white/20 text-[8px] md:text-[10px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] hover:bg-white hover:text-black transition-all"
        >
          Add to Loadout +
        </button>
        <button
          onClick={() => onArchive(shoe)}
          className="flex-1 py-3 md:py-4 border border-white/10 text-[8px] md:text-[10px] font-mono text-gray-500 uppercase hover:text-blue-500 hover:border-blue-500/50 transition-all bg-white/5"
        >
          Archive
        </button>
      </div>
